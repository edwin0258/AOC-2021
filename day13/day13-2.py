...
# Make a visual display of dots based on y coords dict
def displayCoordsY(dotCoords):
    # where final display rows are placed
    display = [[]] * (max(dotCoords.keys()) + 1)

    for y in dotCoords:
        displayRow = ['.'] * (max(dotCoords[y]) + 1)
        for x in dotCoords[y]:
            displayRow[x] = '#'
        display[y] = displayRow
